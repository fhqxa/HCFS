...
%% Train classifiers of all internal levels    
%     levels = max(unique(tree(:,2)));
%     for i = 1:levels
%             c = unique(tree(find(tree(:,2)== i),1))
%             trainLabel = [];
%             trainData = [];      
%                 for t = 1:length(c)
...
%     [predict_label] = FS_topDownSVMLevelPrediction(levels,test_data, modelSVM, tree,feature,numberSel) ;%按层进行预测，shunxin
    %%          Envaluation       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   TIE(k) = EvaHier_TreeInducedError(M,test_label,predict_label',tree);
